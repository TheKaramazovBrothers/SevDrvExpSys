...
int transform_radix2(double real[], double imag[], Uint32 n)
{
    ...
    for (i = 0; i < n; i++)
    {
        Uint32 j = reverse_bits(i, levels);
        if (j > i)
        {
            double temp = real[i];
            real[i] = real[j];
...
int transform_bluestein(double real[], double imag[], Uint32 n)
{
    ...
    for (i = 0; i < n; i++)
    {
        double temp = M_PI * (Uint32)((unsigned long long)i * i % ((unsigned long long)n * 2)) / n;
...
/************************************************************************/
/* for example 001----100 ???¦Ë????    ???change?¦±? ??§³??? ?????????¦Ë????                                                               */
/************************************************************************/
static int32 reverse_bits1(int32 x, Uint32 levels)
{
    int32	result = 0;
    Uint32	i, j;
    //?????????????????????????levels = 2--????1?¦²?10???? levels = 3??????¦²?001-100????levels = 4???????¦²?0001--1000????levels = 5??????????
    //??????????????????????????
    Uint32  change = 0;
    change = levels >> 1;
    //???????????????§Þ????
